...
export default function SignUpAdmin() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(INNERGARDEN_API + `/sign-up/admin`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (response.status === 409) {
          ...
          setSnackbarMessage(
            errorData.detail || 'Internal server error. Please try again later.'
          );
        } else {
          setSnackbarMessage(
            errorData.detail || 'Sign up failed. Please try again.'
          );
        }
